...
### Post-roll condition predicate `test_succeeded` ✅ RESOLVED in codex v2.39.0 / GBF v1.27.0 / CLI emulator v3.22.0 / HTML emulator v3.20.0

**Resolution.** Shipped as Rule 44. `test_succeeded` added to the schema condition enum (the underlying emulator handling and codex prose were already in place for `stat_test`-driven outcomes); `roll_dice.results[range]` gained an optional `outcome: "success" | "failure"` field that sets `state.lastTestResult` on roll resolution, so the FF convention of encoding Test-your-Luck as `roll_dice` with `'lucky'` / `'unlucky'` range keys also drives the predicate. See CHANGELOG → v2.39.0 for the full surface, and codex doc Rule 44 for trigger phrasing + worked examples. A future Warlock re-remediation will re-encode the 12 dropped choices (the §91 case unlocks immediately; the §198 / §333 luck-test cases unlock after tagging the matched ranges with `outcome`; the §131 `if_win` and §350 / §390 / §396 flag-style cases are out of Rule 44's scope and remain tracked under their own gaps).

**Original context (kept for historical reference).** Many Fighting Fantasy sections present a follow-up choice or event whose availability depends on the outcome of the dice roll the player just made — typical phrasing is "If you successfully tested your Luck, turn to A; if not, turn to B" *after* a separate Test Your Luck happened earlier in the same section, or "If you passed the roll, you may also …". The schema has `roll_dice.results[]` per-range targets and `stat_test.success_to / failure_to` direct branches, but no `condition` primitive that says "this choice / event is available iff the last dice resolution succeeded." The Warlock v2.37.0 remediation pass had to **drop 12 conditional choices and events** across the book because there was no valid encoding — the choices are flagged in `metadata.parser_notes` of `claude_session/warlock_fresh_parse_chat39_v237.json`. This is the single highest-impact gap from the Warlock work: 12 narrative branches inaccessible to the player.
...
### Clamped restore action `restore_to_initial` ✅ RESOLVED in codex v2.40.0 / GBF v1.28.0 / CLI emulator v3.23.0 / HTML emulator v3.21.0

**Resolution.** Shipped as Rule 45. The investigation surfaced a nuance the original backlog entry missed: the engine already enforces `initial_is_max` on `modify_stat` (`cli-emulator/play.js:1370`), so the "regain N, up to your Initial" *clamped-ADD* shape is already a solved problem — `modify_stat` + `initial_is_max: true` on the stat declaration. The real gap was the *SET* shape ("STAMINA is restored *to* its Initial total" — no numeric amount stated) which `modify_stat: +999` only approximated. Rule 45 ships the new `restore_to_initial` event for the SET shape and the codex doc carries both halves of the decision so parsers pick the right primitive per phrasing. See CHANGELOG → v2.40.0 for the full surface and codex doc Rule 45 for the decision table + worked examples (Holy Water, Strength/Skill/Fortune potions, Laumspur). A future Warlock comprehensive-review sub-agent migrates §109's Holy Water and the Strength/Skill/Fortune potions to `restore_to_initial`; LW1's Laumspur stays on `modify_stat` per the clamped-ADD half of the decision.

**Original context (kept for historical reference).** Warlock's Holy Water (and several minor potions / spells across the FF series) restore the player's STAMINA *up to their Initial total* — i.e. add N, clamping at initial. The schema has `modify_stat` (unbounded additive delta) and `set_initial_to` (sets the initial cap itself), but no action that adds-with-clamp-to-initial. The Warlock parse encoded these as `modify_stat: +999` with a comment, relying on a manual clamp the engine doesn't actually apply. Result: drinking Holy Water briefly pushes STAMINA above its Initial cap in the stats panel until the player takes ordinary damage.
...
### Combat interrupt on Nth player wound ✅ RESOLVED in codex v2.42.0 / GBF v1.30.0 / CLI emulator v3.24.0 / HTML emulator v3.22.0

**Resolution.** Shipped as Rule 46 (Pausable / Resumable Combat as Active State). The original backlog entry framed this as a single "soft loss" field (`lose_to_after_wounds`); the chat-40 fresh-parse design discussion expanded scope to cover BOTH directions (player-takes-Nth-wound interrupt AND player-deals-Nth-wound interrupt) AND the resume case where the SAME fight continues at a later section with preserved enemy STAMINA (§173 + §24's "the fight goes on" loop). The implemented schema fields are `combat.interrupt_after_player_wounds: {count, target}` (wounds taken — the original entry's case) and `combat.interrupt_after_enemy_wounds: {count, target}` (wounds dealt — the §41 first-wound-exit companion case), plus a new `state.activeCombat` object preserving paused-combat state across the interlude and a new `combat.mode: "start" | "resume" | "modify"` lifecycle field that drives whether a downstream combat creates a fresh fight, continues the paused one, or just adjusts the snapshot in place (wandering-monster pattern). See CHANGELOG → v2.42.0 for the full surface, codex doc Rule 46 for the specification + worked examples, and the design-rationale subsection for why we chose stateful activeCombat (Design B) over the original single-field shape (Design A — would have ossified into a series of one-off fields as new mid-fight-state patterns surfaced). A future Warlock comprehensive-review sub-agent migrates §41 and §173 + §24 to the canonical Rule 46 encoding.

**Future activeCombat extensions** (queue when a second use case for each surfaces):

- **Wandering-monster determination** via `mode: 'modify'` with a roll-table-driven `enemy_ref` swap, followed by a downstream `mode: 'resume'` combat that engages the rolled enemy. The schema field `combat.current_health` (added with Rule 46) pre-populates the chosen enemy's starting HP on the modify-mode event. Pre-Rule-46 this pattern was expressible via `roll_dice` + per-result combat events (one branch per enemy); Rule 46 lets it collapse to a single combat event when the enemy roster is too large to enumerate cleanly per-range.
- **Mid-fight narrative interludes** (player gets a vision, drops an item, dialogue beat) — pause via an interrupt threshold, navigate to a narrative section, resume via `mode: 'resume'` on a continuation combat. Same shape as §173 + §24 but the interlude carries no recurring loop (just a one-shot story beat).
- **Multi-stage / transforming boss fights** where the enemy "phases" mid-fight (HP threshold or wound-count threshold triggers a form change). Encode the first form as a `mode: 'start'` combat with an `interrupt_after_enemy_wounds` set to the phase-change threshold, the transformation section as a `mode: 'modify'` event swapping `enemy_ref` to the new form (preserving the carried-over `currentHealth` or overriding via `current_health`), and the continued fight as a `mode: 'resume'` combat. The wound counters reset per resume so each phase gets its own interrupt window — useful for "after Nth wound on phase 2, fight ends and the boss flees" shapes.

**Original context (kept for historical reference).** Warlock §173's Wight fight has a mechanic: every successful enemy hit on the player is a "wound," and after the **third** wound the fight ends and the player is dragged off to §24 regardless of remaining stamina. The schema's `combat` event has `end_after_rounds` (Rule 38) and `damage_caps` (Rule 32) but no field for "interrupt and navigate to <sid> after the Nth player-side wound." The Warlock orphan-recovery pass encoded this as a conditional choice on §173 reading "If during the fight you were wounded a third time, turn to §24" — relying on the player to honestly self-count wounds. The Wight fight runs as an ordinary combat in the engine.
...
**Workaround now documented (codex v2.41.0 / schema v1.29.0, doc-only).** The mirror-choice pattern itself is documented in `codex.schema.json` (`section.choices` description, plus cross-references on `stat_test.success_to`/`failure_to` and `input_number.target = "computed"`) and in the codex doc Step 7 verification walkthrough ("Reachability mirror choices (when STRANDED is a false alarm)"). This is the transitional documentation: parse sub-agents reading the schema or the codex doc encounter the workaround and the canonical `text`-prefix convention (`"(reachability: ...)"`) for marking mirror choices. When this backlog entry ships and the tool extension lands, that documentation gets removed in the same commit alongside any mirror choices remaining in the maintained books.
