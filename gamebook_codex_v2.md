...
### Rule 30: Permanent Stat Modification — `modify_initial` for Delta Changes, Script for Caps
...
**Cap-style permanent reduction (script-event workaround).** Some books describe permanent reductions as *caps* rather than deltas — Windhammer §440 ("from now on your STRENGTH cannot exceed 11") and §533 (similar ENDURANCE cap). The cap is a hard ceiling at an absolute value, not a relative delta. The cap might leave a player whose current STRENGTH is 9 unchanged, but a player whose current STRENGTH is 13 must drop to 11. There is no `modify_stat.set_initial_to: <value>` field in the current schema (v1.11.0 — tracked as a v2.16.x backlog item); until that ships, the canonical encoding is a `script` event that performs the cap clamp explicitly:

```json
{
  "type": "script",
  "code": "if state.initialStats.Strength > 11 then state.initialStats.Strength = 11 end\nif state.stats.Strength > 11 then state.stats.Strength = 11 end"
}
```

(The script reads and writes `state.stats` / `state.initialStats` directly using the section-level Lua sandbox per Section 7.6's "Lua sandbox available in `script` events" subsection.) This is verbose but correct, and the cap behavior is mechanically faithful to the source text. When the schema gains `set_initial_to`, the encoding migrates to a single `modify_stat` event; until then, document the cap with a short `reason` field on the script event so future maintainers can find these sites for the migration.
...
**Rule 30 (Permanent stat modification — `modify_initial` for deltas, script for caps).** For every `modify_stat` event in the book, I read the source-text language for the section that emits the event. If the text uses *permanently* / *forever* / *initial* / *for the rest of your adventure* / *cannot recover*, the event carries `modify_initial: true` so both `state.stats[stat]` and `state.initialStats[stat]` are adjusted and `initial_is_max` clamping prevents healing from restoring beyond the new ceiling. If the text describes a transient loss with no permanence language, `modify_initial` is omitted (or false). For absolute stat caps ("your STRENGTH cannot exceed 11 from now on") that have no delta form in the source text, I encoded a `script` event that clamps both `state.initialStats[stat]` and `state.stats[stat]` to the cap value (until schema v2.16.x's `modify_stat.set_initial_to` ships and these sites can migrate to a single `modify_stat` event). NO `modify_stat` event whose source text says *permanently* / *initial* lacks `modify_initial: true`; NO `modify_stat` event whose source text describes a transient loss carries `modify_initial: true` over-eagerly.
