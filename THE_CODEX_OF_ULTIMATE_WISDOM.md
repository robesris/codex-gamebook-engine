...
> ⚠ **The codex is technical; the user is not.** This document is dense with implementation vocabulary — schema field names, version pins ("Rule N v2.X.Y"), file/line references (`cli-emulator/play.js:2902`), regex syntax, validator-check identifiers, JSON snippets. **All of that is internal reference material for YOU.** The user is a gamebook reader and curator — somebody who owns the book, wants a faithful digital encoding, and will play it in the emulator. They are NOT assumed to know what a `triggered_effect` is, what `Rule 36` refers to, what `book.rules.provisions.heal_amount` resolves to, or what `damage_set: { direction: outgoing }` means. Reading this codex MUST NOT shift your model of the user toward "they want technical depth." It is exactly the opposite signal: the codex is dense BECAUSE the engine is doing the technical lifting so the user doesn't have to.
>
> In every user-facing turn — parsing, remediation, polishing — translate from codex/schema vocabulary into the source-text vocabulary the user already knows. The book talks about "the iron shield," "the Manic Beast," "STAMINA points," "leaving behind one item of equipment." Your user-facing text uses those terms. The schema's `triggered_effects`, `gate_roll`, `applies_on`, `rules.navigation_transforms` — stay in the JSON file and in this codex, not in your messages to the user. The full discipline lives in Step 2c (basics-mode rules) and §12.1 (the cardinal rule, originally written for the remediation pass but applies to every user-facing turn).
...
13. **Surface engine / schema / codex implementation vocabulary.** See "Codex density is not user technicality" below for the full list and worked examples — but in short: no schema field names, no version pins ("Rule N v2.X.Y", "schema v1.36+"), no file/line references (`cli-emulator/play.js:2902`), no internal validator-check identifiers (`mechanic-verbs-in-note`, `disarmament-without-event`), no JSON snippets in the text body unless the user asked for the exact encoding, no event-type names (`triggered_effect`, `gate_roll`, `route_by_flag`, `damage_set`, `instant_death`, `navigation_transforms`), no condition-predicate names (`has_flag`, `has_item_with_property`). The agent's mental model uses these; the user's doesn't.
...
7. Have I used any schema, codex, version, or implementation vocabulary the user didn't introduce first? (Yes → translate to source-text terms.)

#### Codex density is not user technicality

The codex doc you are reading right now is dense with implementation vocabulary — schema field names, version pins, file/line references, regex syntax, validator-check identifiers, JSON snippets. This density tells you NOTHING about how technical the user is. The codex is your reference material; the user is a gamebook reader and curator. **Do not let reading this dense document shift your model of the user toward "they want technical depth." It is the opposite signal.** The codex is dense BECAUSE the engine is doing the technical lifting so the user doesn't have to.

The discipline: in every user-facing turn, translate from the codex/schema vocabulary you use internally into the source-text vocabulary the user already knows. The book talks about "the iron shield," "the Manic Beast," "STAMINA points," "Provisions," "leaving behind one item of equipment." Your user-facing text uses those terms.

**Concrete BAD vs GOOD examples** (same actual work, basics mode):

BAD (treats the user as a fellow engineer reading the codex):
> §131's `eat_meal` event currently has `note: "May eat Provisions but must share - only gain 2 STAMINA instead of 4"` but the `heal_amount` field isn't set, so the global default of 4 applies. Per the v1.34+ schema, `eat_meal` carries a per-event `heal_amount` override that both emulators honor via `event.heal_amount ?? book.rules?.provisions?.heal_amount ?? 4` (see `cli-emulator/play.js:2902`). This is the Rule 49.1 catch-net pattern. Add `heal_amount: 2`?

GOOD (uses the source-text terms the user already knows):
> §131 says you only gain 2 STAMINA from sharing a meal with the dwarves, not the normal 4. The parse doesn't enforce that yet. Fix it?

The bad version surfaces FIVE pieces of implementation vocabulary (`eat_meal`, `heal_amount`, `schema v1.34+`, `Rule 49.1`, `cli-emulator/play.js:2902`) that the user has no use for. The good version surfaces the SOURCE-TEXT FACT (the dwarves cap STAMINA at 2 instead of 4) and the QUESTION (fix it?). The user can answer the good version without ever reading the codex.

**The translation rule.** When drafting a user-facing turn, ask: *"Could the user answer this question by re-reading the source-text passage, without ever opening this codex?"* If no, you've leaked implementation vocabulary into the question — rewrite using source-text terms.

**Things the user DOES know** and can be referenced freely: section numbers (§131, §439), section names if the book uses them ("the Maze of Zagor"), stat names from the rules section (STAMINA, SKILL, LUCK, ENDURANCE, SHADOW, COMBAT SKILL — whatever the book uses), item names from the book (iron shield, Provisions, gold pieces, the Y-shaped stick), enemy names (Manic Beast, Giant Hornet, Grognag), game-rule terms the book itself defines ("Test your Luck", "Attack Strength", "Action Chart", "Equipment List").

**Things the user does NOT know** and must NOT appear in user-facing text unless they introduced the term first: schema field names (`heal_amount`, `accept_set_flag`, `interrupt_after_player_wounds`), event-type identifiers (`triggered_effect`, `gate_roll`, `route_by_flag`, `damage_set`, `instant_death`, `eat_meal`, `prompt_choice`, `choose_items`, `navigation_transforms`), condition predicates (`has_flag`, `has_item`, `has_item_with_property`, `has_equipped_with_property`), version pins ("Rule 36 v2.51.0", "schema v1.36+"), file/line references (`cli-emulator/play.js:2902`, `index.html:4684`), validator-check identifiers (`mechanic-verbs-in-note`, `disarmament-without-event`, `loss-in-choice-text`), Lua API ("the `combat.vars` scratch table", "`result.combat.vars` writeback"), Rule numbers (saying "per Rule 49.1" is fine for the agent's mental model but not for the user — they don't have the rule catalogue memorised). If you need to discuss one of these because the user explicitly asked, that's the exception that proves the rule; otherwise stay translated.

**Validator output is the one exception.** When the validator surfaces a schema error or a soft-check finding that the user must see to act on it, render the raw finding verbatim in a code block. Don't paraphrase validator output into source-text terms (that loses precision and the user may want to grep the message). But ALSO don't editorialise around it with more schema vocabulary — let the raw output speak, frame it with one source-text sentence above ("The validator flagged §131 with two findings:" + code block + "Want me to fix?").
...
#### Rule 36 extension v2.50.0 — `applies_on: "double"` keyword + `instant_death` effect verb

Surfaced during a Creature of Havoc (FF#24) fresh-parse in an incognito chat. Three CoH enemies (Giant Hornet, Manic Beast, Ophidiotaur) have per-round stateful combat rules that the declarative `triggered_effects` layer couldn't express. All five gaps were filed in `grimoire-engine-books/known_issues.md` under "Creature of Havoc (FF#24) parse — engine follow-ups to file as GitHub issues"; this section documents the two that ship together in schema v1.35.0 / CLI emulator v3.30.0 / HTML emulator v3.26.0.

**Gap 1 — `applies_on: "double"` keyword.** Before v1.35, `gate_roll.applies_on` accepted a single int (`"6"`) or an inclusive range (`"1-2"`), both matched against the SUMMED total. It could not express *"the two dice showed the same face"*, an extremely common FF/Fighting-Fantasy idiom (instant-kill on doubles, fixed-damage on doubles, etc.). Cross-book frequency: ~18 textual hits across 4 of 6 first-party books before the fix.

Two new string forms, both matched against the PER-DIE rolls array (not the summed total):
- `"double"` — fires when all dice in `gate_roll.dice` show the same face.
- `"double:N"` — fires when all dice show face N (e.g. `"double:6"` = "two 6s"). Use when the source-text trigger is specific to a particular pair rather than any double.

Both forms require `gate_roll.dice` to roll ≥2 dice (`2d6`, `2d10`, `3d6`, etc.) — a single-die `gate_roll.dice: "1d6"` with `applies_on: "double"` never fires, since there is no second die to compare against. The single-int and range forms are unchanged from v1.34 and earlier; existing books need no migration.

Canonical encoding for the CoH Hornet sting half (the "enemy rolls doubles → player dies" half; the "UNLESS player also rolled doubles" half is filed as Gap 3, not yet shipped):
...
"triggered_effects": [{
  ...
  "reason": "Giant Hornet sting on doubles"
}]
```

**Gap 2 — `instant_death` effect verb.** Before v1.35, lethal triggers had to be encoded as `damage_set: { direction: outgoing, value: 9999 }` — capability was fine, but semantic clarity was poor (the event type and log message both read as a damage-set with an arbitrary large number, not as an intentional kill). Cross-book frequency: ~13 textual hits across 2 of 6 first-party books before the fix.

New effect verb in the Rule 36 damage-flow op enum:

```json
{ "type": "instant_death", "direction": "outgoing" }   // kills the player this round
{ "type": "instant_death", "direction": "incoming" }   // kills the enemy this round
```

Semantics: sets the per-round damage in `direction` to `Number.MAX_SAFE_INTEGER`, semantically meaning *"the target dies this round"*. Operates in the same SHIFT_OPS pipeline pass as `damage_set` — cappable by later triggered `damage_cap` ops, though no current source text caps an "unavoidable" lethal effect. Requires no `value` field; the lethal value is implicit. Symmetric across directions.

When to prefer `instant_death` over `damage_set`:
- Source text says *"you die"*, *"instantly killed"*, *"is dead"*, *"is killed"* → use `instant_death`.
- Source text says *"you take 9 damage"*, *"the dragon's breath does 6 damage"* → use `damage_set` (or `damage_delta` for a shift).
- Source text says *"the vorpal blade kills the dragon outright"* → use `instant_death`.
- Source text says *"the wand does 1d6 damage"* → use `damage_delta` (combined with a separate roll, or via the round_script if the dice is complex).

The two verbs are functionally interchangeable in the no-cap case, but `instant_death` makes the source-text intent legible in the JSON and produces a clearer playthrough log (`R36 instant_death (item:wand_of_death): outgoing → lethal` instead of `R36 damage_set (item:wand_of_death): outgoing 0 → 9999`).

**Gaps 3, 4, 5 status.** Gap 4 was originally documented as "round_script is faithful here" — this claim was empirically false at CLI v3.30.0 / HTML v3.26.0 and was retracted in v2.51.0 (see the sub-section below). Gaps 3 and 5 are still filed in `known_issues.md` for follow-up:
- Gap 3 — cross-component same-round predicates (single known site: CoH Hornet's "UNLESS player also rolled double" half). Re-evaluate when a second occurrence surfaces.
- ~~Gap 4~~ — **resolved in v2.51.0.** See the Rule 36 extension v2.51.0 sub-section below.
- Gap 5 — optional mid-combat player choice (≥3 known sites: CoH Ophidiotaur + Warlock §63 / §282). Flagged as **design-priority candidate** for the next time the pattern surfaces — recurrence in Warlock suggests this is the most likely next implementation among the deferred gaps. Worth a focused design session before implementation: the trigger condition (player-initiated optional pause) is distinct from the engine-driven interrupts already in the Rule 38 / 48 family.

**Verification.** Both new shapes have full coverage in `tests/run.js` (`Rule 36 v2.50.0:` block): doubles-fires-on-pair, doubles-skips-on-non-pair, `double:N` specificity, regression of pre-v1.35 single-int and range forms, instant_death player-direction kill, instant_death enemy-direction kill, instant_death + doubles composition (the CoH Hornet shape), schema-additive pre-v1.35 books validate. 9 new tests, all green.

#### Rule 36 extension v2.51.0 — `round_script` cross-round persistence (closes Gap 4)

**Retraction.** The v2.50.0 documentation of Gap 4 included the claim *"round_script is faithful here"*. That claim was empirically false at CLI v3.30.0 / HTML v3.26.0 — and was never tested. Independent reproduction at the pinned commit by the CoH parser (`tests/run.js` `Rule 36 v2.51.0:` block now locks it in):
- The per-round `combat` table (combatData) was rebuilt fresh every round; arbitrary keys a script wrote to `combat` were present in `result.combat` but the engine only read back `last_result` and `last_damage`. Anything else was silently dropped.
- `game_state` was round-tripped for SECTION scripts (`runScriptEvent`) but the COMBAT round path never persisted `result.game_state`. Scripts that wrote to it saw the writes vanish next round.
- The engine maintained `state.combat.woundsDealt` / `woundsTaken` (per-fight counters driving Rule 46 interrupts) but did not pass them INTO the script's `combatData` — so a round_script could not even read them, let alone gate on them.

Net effect: there was no slot — readable or writable — that a round_script could use to carry state from one round to the next. The CoH §263 Manic Beast rage buff (and any other "buff depends on what happened last round" idiom) was not encodable. The codex's claim contradicted the engine.

**Fix shipped in v2.51.0.** Three additions to the per-round `combatData` passed to round_scripts, with the corresponding wiring in both emulators:

| Field | Direction | Description |
|---|---|---|
| `combat.vars` | read/write | Per-fight scratch table. Initialised empty on combat start; restored from `state.activeCombat.vars` on Rule 46 resume. The engine reads `result.combat.vars` back after each round and persists it to `state.combat.vars`; scripts that ignore the field are unaffected (the writeback no-ops when the field is missing or not an object). Round-tripped through `pauseCombat`'s snapshot so a paused/resumed combat preserves the scratch table. |
| `combat.wounds_dealt` | read-only | Engine-maintained monotonic counter — incremented when the prior round's `last_result` is `player_wounds_enemy`. Resets to 0 on every combat start AND on every Rule 46 resume (each resume opens a fresh interrupt window — matches the existing Rule 46 semantics for `interrupt_after_enemy_wounds` checks). Available to scripts that need to gate on "have I wounded the enemy yet?". Note: the *prior* round's outcome is visible — a round_script reading `combat.wounds_dealt` sees the state BEFORE this round's own bookkeeping runs. |
| `combat.wounds_taken` | read-only | Same shape as `wounds_dealt` but for `enemy_wounds_player` rounds. |

The new fields are runtime Lua-context only — **no schema change.** Books migrate by editing their round_script Lua source, not by editing the JSON. Pre-v2.51 round_scripts that don't reference any of the three new fields continue to work unchanged (verified by the `round_scripts that ignore combat.vars are unaffected` regression test).

**Canonical encoding for the CoH Manic Beast rage buff (§263).** The source-text rule: *"each time you inflict a wound on it, its anger rises for the next Attack Round, and you must add 2 to the dice roll when it rolls for Attack Strength. If you wound it again during that next round it retains the bonus; if you do not, its temper returns to normal and the bonus is lost."*

```lua
-- Initialise the scratch table on first entry
combat.vars = combat.vars or { rage_active = false }

-- Apply this round's attack-strength math, with the rage bonus when active.
-- The book-canonical FF formula is (SKILL + 2d6 rolled) compared between
-- player and enemy; the rage buff adds +2 to the ENEMY's dice roll.
local enemy_bonus = combat.vars.rage_active and 2 or 0
local player_as = player.skill + roll('2d6')
local enemy_as  = enemy.skill  + roll('2d6') + enemy_bonus

if player_as > enemy_as then
  combat.damage_to_enemy = 2
  combat.damage_to_player = 0
  combat.last_result = 'player_wounds_enemy'
  combat.vars.rage_active = true   -- wound dealt → rage active next round
elseif enemy_as > player_as then
  combat.damage_to_enemy = 0
  combat.damage_to_player = 2
  combat.last_result = 'enemy_wounds_player'
  combat.vars.rage_active = false  -- no wound dealt this round → rage clears
else
  combat.damage_to_enemy = 0
  combat.damage_to_player = 0
  combat.last_result = 'no_damage'
  combat.vars.rage_active = false  -- tie counts as "did not wound" → clears
end
```

The retain-or-clear nuance (rage stays true on consecutive wounds, clears on any non-wound round) is naturally expressed in the conditional: `rage_active` is assigned `true` only on the wound branch, so any other branch clears it.

**Relationship to the deferred Gap 4 design (`on_round_start` trigger).** The original Gap 4 ask had two halves: (a) a per-fight scratch slot, (b) a declarative `on_round_start` triggered_effect hook. v2.51.0 ships only half (a). The declarative hook (`triggered_effects` with `trigger: on_round_start` that read/write `combat.vars`) is filed for future consideration; the round_script-side `combat.vars` solution is sufficient for the §263 shape and is the smaller, lower-risk change. Books that need the buff-as-data shape can still encode it via round_script + `combat.vars` until the declarative version lands.

**Verification.** Six new tests in `tests/run.js` (`Rule 36 v2.51.0:` block): vars persists across rounds, wounds_dealt / wounds_taken are script-readable, the CoH Manic Beast shape produces correct retain/clear semantics across four rounds, vars round-trips through Rule 46 pause/resume, ignore-the-field scripts are unaffected. Cross-book validator sweep: 0 schema errors across all six first-party books (no schema change, so the sweep is a sanity check rather than a regression risk).

#### Rule 36 extension v2.55.0 — `round_script` mid-round player-choice pause (closes CoH Gap 5)

Surfaced live during the Creature of Havoc parsing session at §238 (Ophidiotaur). Source-text: when the Ophidiotaur rolls a double for its attack-strength dice, the player MAY opt to Test their Luck to avoid the resulting tail sting — but they don't have to. Faithful encoding requires the engine to pause mid-round and surface a yes/no choice. Pre-v2.55, no such hook existed: the `round_script` ran start-to-finish on its own each round, so the parser was forced to hard-code an auto-resolve rule ("test when LUCK is healthy") that wasn't in the source. The same limitation also covered Warlock §63 / §282 instinct-roll overlays.

**Schema:** no change. This is a runtime convention on the per-round Lua `combat` table, parallel to v2.51.0's `combat.vars` / `wounds_dealt` / `wounds_taken`.

**Round_script API additions** (one new field, two new lifecycle states):

- **`combat.pause_for_choice`** (write). When set by the script during a round, the engine pauses the round mid-execution. Shape: `{ prompt: "...", accept_label: "...", decline_label: "..." }`. Strings only; all three fields optional (defaults: "Make a choice", "Accept", "Decline" — though the script should always supply at least `prompt`). Setting this and returning is a *pause request*, not a fallthrough to round resolution: the engine does NOT apply damage, does NOT update `last_result`, does NOT increment wound counters, does NOT dispatch on_combat_round triggers, and does NOT advance `combat.round`. The round is suspended.

- **`combat.player_choice`** (read). On the resumed re-run of the round_script after the player resolves the choice, this field carries the answer: `"accept"` or `"decline"`. On the INITIAL call (round entry) the field is `null`. Scripts structure their pause logic as: detect the trigger condition; if `combat.player_choice == nil`, set `combat.pause_for_choice` and `return`; if set, branch on the answer.

**Captured-rolls replay.** The script's `roll()` calls during phase 1 (the initial call that requested the pause) are captured by the engine. When phase 2 resumes, the engine replays those rolls as forced rolls so phase 2's dice match phase 1's exactly. This matters because most pause conditions are dice-driven ("the enemy rolled a double") — re-rolling in phase 2 would break the condition. Phase 2 may call `roll()` MORE times than phase 1 did (e.g. an extra Luck-test roll in the accept branch); those extra rolls use real RNG (the captured queue runs out and rollDice falls through). The capture/replay is transparent to the script writer — write the script naturally; the engine handles it.

**Combat.vars writeback persists across the pause.** Consistent with v2.51.0 semantics: a phase-1 write to `combat.vars` is read back into `state.combat.vars` before the engine pauses. Phase 2's script reads the same vars it wrote in phase 1. Useful for stashing intermediate analysis the phase-2 branch wants to use.

**State management.** State.pause receives `{ type: 'combat_choice', prompt, accept_label, decline_label, captured_rolls }`; state.combat is preserved as the live combat object (not torn down). On the user's accept/decline action, `state.combat.pendingPlayerChoice = { action, capturedRolls }` is set, the pause is cleared, and runCombatRound is re-invoked. The runner detects pendingPlayerChoice at its top, skips the `combat.round++`, uses the captured rolls as forcedRolls, and sets `combat.player_choice` in the combatData passed to runScript.

**Canonical encoding for CoH §238 Ophidiotaur tail-sting:**
...
if enemy_doubled and combat.player_choice == 'accept' then
  ...
  if luck_roll.total <= player.luck then
    combat.damage_to_player = 0           -- evaded
    combat.last_result = 'no_damage'
  else
    combat.damage_to_player = 4           -- worse damage on Unlucky
    combat.last_result = 'enemy_wounds_player'
    combat.last_damage = 4
  end
elseif enemy_doubled and combat.player_choice == 'decline' then
  combat.damage_to_player = 2             -- the standard sting
  combat.last_result = 'enemy_wounds_player'
  combat.last_damage = 2
else
  -- Non-double rounds resolve normally — standard attack-strength comparison.
  -- ... existing per-round math ...
end
combat.damage_to_enemy = 0  -- (omitted from this excerpt for the doubles branches; fill in per the book's enemy-wound math)
```

The script reads naturally top-to-bottom. The pause-request branch fires only when the trigger condition holds AND no answer has been collected yet. Phase 2's accept and decline branches make the same `ead = roll('2d6')` call as phase 1 — and get the same dice back via captured-rolls replay, so the `enemy_doubled` check holds on the re-run.

**Relationship to Rule 38 / Rule 48 interrupts.** The Rule 38 round-gate interrupt (flee available after N rounds) and Rule 48 round-cap interrupt (combat ends after N rounds) are engine-driven — the engine decides when to fire based on the round counter. Rule 36 v2.55.0's pause is script-driven — the script decides when to pause based on the round's dice. The two families coexist: a fight can have round-cap interrupts AND mid-round player-choice pauses, and both fire independently.

**Triage note.** Single-site recurrence at decision time (CoH §238) but with cross-book pattern recurrence: the FF "Test your Luck to avoid" overlay appears in Warlock §63 / §282 and analogous idioms recur across the series. Shipped because (a) the live parsing session hit it and the auto-resolve workaround was unfaithful, (b) the implementation cost was bounded (~200 lines across CLI + HTML + script-runtime + tests), (c) the design reused the v2.51.0 combat.vars infrastructure cleanly, and (d) the same convention generalizes to any "round_script needs to pause for a player decision" idiom.

**Verification.** Seven new tests in `tests/run.js` (`Rule 36 v2.55.0:` block): pause fires when script sets `combat.pause_for_choice`; accept resume re-runs with `player_choice='accept'`; decline resume re-runs with `player_choice='decline'`; captured rolls replay produces matching dice across phases (5 attempts to defeat RNG luck); the §238 Ophidiotaur shape produces correct damage in non-double / accept-lucky / decline-take-sting paths; combat.vars writeback persists across the pause; vanilla scripts that never set pause_for_choice are unaffected. 132/132 total tests pass.
...
**The source-text shape this addresses.** FF Warlock §333's Vampire flee mechanic. After a number of rounds of combat (6, in this case) the player may attempt to flee, gated on a Test-your-Luck. Lucky succeeds and the player exits; Unlucky forces another fixed window of rounds against the same opponent before another flee attempt is possible. A special sub-branch fires when the Luck-test roll is at the high end (11 or 12) AND the player was Unlucky — that branch routes to a different downstream section. The fight has to PAUSE for the Luck-test decision while preserving the enemy's remaining STAMINA, frozen modifiers, and round counters; on Unlucky-non-special the SAME fight resumes, not a fresh encounter.
...
### Rule 49.1: Stat-name-in-note catch-net + parser attention to stat-name and rule-cased tokens

Surfaced when FF Warlock §131 (the shared-meal half-heal section) was found to be silently un-enforced. The source text — *"You may, if you wish, eat a meal from your Provisions, but you will have to share it with them and thus will only gain half the normal STAMINA points"* — was encoded as `{ type: "eat_meal", required: false, note: "May eat Provisions but must share - only gain 2 STAMINA instead of 4" }`, leaving the heal_amount at the global default (4) instead of using the per-event `heal_amount` override the schema already provided. The Rule 49 `mechanic-verbs-in-note` catch-net from v2.45.0 did NOT fire — its regex set (`must drop`, `also lose`, `in addition to`, etc.) didn't include the §131 verbs (`must share`, `only gain`, `instead of N`). Two complementary fixes:

**Extension 1: extended `mechanic-verbs-in-note` regex vocabulary.** The Rule 49 verb set now also catches:
- `\bmust\s+share\b` (the §131 explicit verb)
- `\bonly\s+(?:gain|get|receive|restore|lose|recover)\b` (the §131 quantifier — "only gain 2 STAMINA")
- `\binstead\s+of\s+(?:the\s+)?(?:normal|usual|standard|\d+)\b` (the §131 comparison — "instead of 4")
- `\b(?:half|halve|halved|double|doubled)\s+(?:the\s+|your\s+)?(?:normal|usual)\b` (the §131 halve-the-normal phrasing in source-text wording)

These are conservative — each requires a constraint-shaped phrase, not a bare stat-name mention. Sub-agents who must restate a §131-shape constraint in a note (e.g. a partial migration where one half is encoded and the other is still pending) should expect the catch-net to fire and either complete the migration or temporarily disable the note.

**Extension 2: new soft check `stat-name-in-note`.** Walks every event's `note` field and flags any note that mentions a declared stat name (from `book.rules.stats[].name`, plus the cross-engine alias set: `stamina, skill, luck, endurance, combat_skill, willpower, provisions, gold`) AND contains a digit AND contains a constraint-shaped word (`only, just, must, may only, can only, instead of, half, halve, double, share, forfeit, forced to`). Tuned to be conservative — requires ALL THREE signals so that benign parser commentary does NOT trip:
- `"First word 'STAMINA' confirmed from PDF"` → stat name present, no digit, no constraint → does NOT flag.
- `"STAMINA cap remains 24"` → stat name + digit, but no constraint word → does NOT flag.
- `"only gain 2 STAMINA instead of 4"` → all three signals present → DOES flag.

Cross-book scan against the six first-party books (Warlock, LW1, Grailquest, Windhammer, Beware-of-the-Purple-Peanut-Butter, Forest of Twisted Dreams) surfaces exactly ONE finding: Warlock §131. Precision-tight at deploy.

**The parser-attention principle (the user-noted insight):** any text containing a declared stat name (in any casing) or a differently-cased rule token (proper-noun-cased English nouns the rules section defines as game-mechanical — `Backpack`, `Weapon`, `Provisions`, `Equipment List`, `Action Chart`, `Special Ability`, etc.) almost certainly carries mechanical weight in the source. Sub-agents performing first-pass parsing should treat such tokens as **mandatory-encode signals**, not flavor: if the source says *"you must use a Special Ability"* or *"you lose half your STAMINA"* or *"adjust your Equipment List"*, the events array MUST encode the constraint structurally. Stashing the constraint in a `note` and leaving the events array un-enforced is the exact failure mode this rule exists to prevent. The catch-nets (Rule 49 `mechanic-verbs-in-note` + Rule 49.1 `stat-name-in-note`) are the validator-side safety net for when the parsing instinct slips.

**Resolution shape for §131-style heal-amount variation.** The schema already declares `heal_amount` as a per-event integer override on `eat_meal` (alongside the global default in `provisions_rules.heal_amount`); both emulators honor it via `event.heal_amount ?? book.rules?.provisions?.heal_amount ?? 4`. The §131 encoding becomes a one-field deviation:

```json
{ "type": "eat_meal", "required": false, "heal_amount": 2 }
```

The flavor text ("share with them") lives in the section's `text` field where it belongs — not in a `note`. Use the highest-level event field that expresses the variation; only drop to raw `modify_stat` primitives wrapped in a `prompt_choice` when no schema field fits (the principle was first applied in Rule 49 for multi-stat `failure_penalty` and again in Rule 50 for voluntary-trade encodings).

**When NOT to use the override.** If the variation isn't a parameter — e.g. "eating here triggers a SKILL test, on failure no heal" — drop to primitives (`prompt_choice` + raw `modify_stat`) because no schema field expresses that shape. The `eat_meal` event is still the right choice when the only deviation from the global default is the heal amount (or heal stat).

**Verification.** Both catch-nets now fire on §131 with two cooperating findings. After the §131 re-encoding pass, both go quiet and the book has zero Rule 49 / Rule 49.1 soft findings. The negative-test pair (a benign parser-commentary note mentioning STAMINA, and a stat+digit note without a constraint word) is also covered by the test suite to lock in the precision tuning.
...
### Rule 51: Flag-gated global navigation transform (`navigation_transforms`)

Surfaced during the Creature of Havoc (FF#24) parse at §439. A companion (Grognag "Grog" Clawtooth) joins the party and, while present, rewrites the player's navigation: *"if you turn to any reference ending in a 7 (e.g. 247), deduct 52 from it and turn to this new reference."* The transform is active from §439 (where the companion joins) until §235 (where the companion dies). Without a first-class primitive, the only encodings were (a) ~37 per-section manual edits with subtly wrong semantics, or (b) a `set_flag has_grog` at §439 with no enforcement — the route the parser took before filing this gap, leaving 159 destination sections unreachable in play.

**Why existing primitives fail.** Rule 47 `route_by_flag` fires on ARRIVAL at a destination section and routes among a per-section enumerated list. It cannot intercept the outgoing navigation, and cannot express a modulo-class rewrite. Conditional choices would require editing every potentially-targeting source section. The mechanic the source describes is a wholesale transform of the navigation RESULT — gated on a single flag — that applies to all paths uniformly.

**Schema addition (v1.36+, schema-additive — pre-v1.36 books validate unchanged):**

New optional field `rules.navigation_transforms[]` at book level:
  ...
  "navigation_transforms": [
    {
      ...
      "apply": { "offset": -52 },
      "reason": "Grognag Clawtooth companion: -52 on references ending in 7"
    }
  ]
}
```
...
Each entry declares:
- `while_flag` (string, required) — flag that must be on `state.flags` for the transform to fire. Set via `set_flag` at the section that activates the companion/curse/etc.; cleared via `clear_flag` at the section that deactivates it.
- `match` (object, required) — predicate evaluated against the navigation target.
  - `match.target_mod: [divisor, remainder]` — fires when `(numericTarget % divisor) === remainder`. For "any reference ending in 7" use `[10, 7]`. Positive-remainder arithmetic so negative incoming targets are handled correctly. Currently the only documented match shape; future shapes (`target_in: [N, ...]`, `target_range: [lo, hi]`, `target_eq: N`) may be added when a real source-text case justifies them.
- `apply` (object, required) — transform applied when match passes.
  - `apply.offset: N` — signed integer added to the numeric target. For "deduct 52" use `-52`. Currently the only documented apply shape; future shapes (`set_to: N`, `apply_table: {N: M, ...}`) may be added when justified.
- `reason` (string, optional) — human-readable explanation; surfaced in the playthrough log.

**Engine semantics.** The transform runs inside `navigateTo()` BEFORE the destination section is looked up. Every navigation path — choices, computed `input_number` jumps (secret doors), `route_by_flag`, `stat_test.success_to`/`failure_to`, combat `win_to`/`flee_to` — funnels through `navigateTo()` so all paths get the same treatment uniformly. This is intentional: the source text says *"if you turn to any reference ending in 7"* without distinguishing how the player got there, so the transform applies regardless.

**Compose order.** Transforms are evaluated in the order declared in `navigation_transforms[]`; the first match wins. At most one transform fires per navigation. Multiple compounding transforms are not currently supported (no known source-text case); first-match-wins is the conservative semantic.

**Invalid-target fallback.** If the transform's computed result is not a valid section id (e.g., offset produces a negative number, or the source-text rule has an edge case the math doesn't cover cleanly), the emulator logs a warning and navigates to the ORIGINAL target rather than the transformed one. This avoids dead-ending the player on a misconfigured transform; the warning surfaces the misconfiguration to whoever's playing or testing the book.

**Canonical encoding for CoH §439 Grognag.** Three-piece pattern:
...
// rules block (anywhere in the book):
"rules": {
  "navigation_transforms": [
    {
      "while_flag": "has_grog",
...
That's the entire encoding — three additions across two sections and the rules block. The 37 destinations matching `% 10 == 7` while `has_grog` is active are rewritten by the engine; the same destinations resolve normally before §439 and after §235.

**When NOT to use it.** Per-section conditional routes that DON'T fit a modulo-class predicate belong in `route_by_flag` (Rule 47) or in conditional `choices[]`. One-off "if X then turn to Y else turn to Z" patterns are choice-level; `navigation_transforms` is for wholesale rewrites that span many destinations and would otherwise require per-section editing.

**Verification.** Both emulators implement the transform at the `navigateTo()` chokepoint. Seven new tests in `tests/run.js` (`Rule 51 v2.53.0:` block) cover: fires-when-flag-and-match, does-not-fire-without-flag, does-not-fire-without-match, fires-through-choice-navigation, multiple-transforms-first-match-wins, invalid-target-fallback, schema-additive pre-v1.36 books validate. Cross-book schema sweep: 0 errors across all six first-party books (no existing book uses the field yet; the additive shape is backward-compatible).

**Cross-book frequency.** 1 confirmed site (CoH §439). Pattern cue: *"companion/curse alters reference numbers by a fixed offset on a digit class"* — a recurring FF idiom (companions, curses, maze renumbering). Shipped despite single-site recurrence because (a) the implementation cost is small (~50 lines across both emulators including validation), (b) the blocking severity is HIGH (159 sections unreachable until fixed — not a polish issue), and (c) the user's design sketch was clean enough to drop in nearly verbatim.
...
**This rule applies to every user-facing turn, not just remediation.** The same discipline holds during initial parsing, mid-parse questions, polishing, and any other phase. Step 2c's "Codex density is not user technicality" sub-section is the full statement; this section is the remediation-pass-specific framing with worked examples drawn from the remediation finding categories.
...
**Codex v2.58.0 / GBF schema v1.36.0 / CLI emulator v3.33.0 / HTML emulator v3.32.0** (HTML emulator UX bugfix: clicking the Use button more than once on an item whose `on_user_use` effect renders content into game-content (canonical case: a document item whose script log-prints the document's text) stacked duplicate output on top of the section, burying the choice buttons under layered content. Two defensive guards added to r36_useItem: a re-entry flag (`window._r36UseItemInFlight`) prevents concurrent dispatch from rapid double-clicks; a pending-script-continue check rejects further Use clicks while the previous Use's script is still awaiting the player's Continue. Surfaced live by the Creature of Havoc browser session immediately after the v2.57.0 `handleEvent` fix landed. No schema change; CLI unchanged. v2.57.0 — HTML emulator bugfix: `handleEvent` was referenced by `r36_dispatchTriggeredEvent` but never defined in the HTML emulator — a copy-paste from the CLI pattern that wasn't completed. Any click on an item's "Use" button when the item carried `on_user_use` triggered_effects crashed with `ReferenceError: handleEvent is not defined`. Bug present since the original Rule 36 wire-up; surfaced live by the Creature of Havoc browser session. Fix: add a 28-line `handleEvent(event)` synchronous single-event dispatcher that mirrors the subset of processNextEvent's switch covering the non-pausing event types the schema's `triggered_effect_action` union permits — modify_stat, set_flag, clear_flag, add_item, remove_item, restore_to_initial, remove_inventory_category, script. Pausing event types (combat, stat_test, roll_dice, eat_meal, choose_items, input_*, return_to_caller) are explicitly schema-forbidden in triggered_effect_action; the dispatcher warns and skips if one is encountered. No schema change; CLI unchanged. v2.56.0 — HTML emulator bugfix: three sites (Rule 51 navigation_transform check, Rule 40 choose_items `on_success_set_flag`, Rule 50 prompt_choice flag set) used Array methods `.includes()` / `.push()` on `state.flags`, which is a `Set` in the HTML emulator (the CLI emulator stores it as an Array — that's why the bug was browser-only). Crashed silently on first navigation for any book using Rule 51 navigation_transforms; would also have crashed any book using Rule 40 `mode: remove` with an `on_success_set_flag`, or any Rule 50 prompt_choice setting a flag. Surfaced live by the Creature of Havoc parsing session — the §439 Grognag navigation rewrite produced a blank screen on first play in the browser while the CLI played the same book fine. All three sites now use `Set.has()` / `Set.add()` correctly. No schema change; CLI unchanged. v2.55.0 — Rule 36 extension v2.55.0 closes CoH Gap 5 — round_script mid-round player-choice pause. New `combat.pause_for_choice = { prompt, accept_label, decline_label }` write field signals the engine to suspend the round; the script returns and the engine pauses with `state.pause.type = 'combat_choice'`. On the player's accept/decline answer, the engine sets `combat.player_choice` in the next combatData and re-invokes the round_script — same round (no `combat.round++`), captured phase-1 rolls replayed as forcedRolls so phase 2's dice match exactly. No schema change (runtime Lua convention). Closes the live-blocking CoH §238 Ophidiotaur tail-sting case where the prior "auto-resolve via fixed rule" workaround was unfaithful; also covers Warlock §63 / §282 instinct-roll overlays and analogous "Test your Luck to avoid" idioms across the FF family. v2.54.0 — user-audience discipline: "codex density is not user technicality." — three additions to keep the parsing agent from leaking schema/codex/version/file-path vocabulary into user-facing text. A prominent callout in SYSTEM INSTRUCTIONS primes the agent BEFORE it reaches the interactive flow: this codex is your reference material; the user is a gamebook reader and curator who is NOT assumed to know what a triggered_effect is, what Rule 36 refers to, or what JSON shapes look like. A new sub-section in Step 2c gives concrete BAD vs GOOD examples (§131 heal_amount example surfaces 5 implementation terms in BAD vs 0 in GOOD), a "things the user DOES know" allow-list (section numbers, stat names, item names from the book, game-rule terms the book defines), a "things the user does NOT know" deny-list (schema field names, event-type identifiers, condition predicates, version pins, file/line refs, validator-check IDs, Lua API), and a translation rule: ask "could the user answer this question by re-reading the source-text passage, without ever opening this codex?" — if no, leak detected, rewrite. New 13th DO-NOT-list entry and new 7th self-check question lock the discipline in at the per-turn level. §12.1 generalised from "the remediation agent" to "every user-facing turn." Documentation-only — no schema, validator, or emulator change. v2.53.0 — Rule 51 — flag-gated global navigation transform. New `rules.navigation_transforms[]` at book level rewrites navigation targets inside `navigateTo()` while a named flag is set: `{ while_flag, match: { target_mod: [divisor, remainder] }, apply: { offset: N } }`. Closes CoH §439 Grognag companion gap ("if you turn to any reference ending in 7, deduct 52") — without the primitive, 159 destination sections were unreachable in play. Applied at the navigation chokepoint so every path (choices, computed input_number jumps, route_by_flag, stat_test targets, combat win_to/flee_to) gets the same treatment. First-match-wins compose order; invalid-target → warn + fallback to original. Schema-additive — pre-v1.36 books validate unchanged. v2.52.0 rewrote Step 2c "Output Verbosity Mode" with prescriptive force. Surfaced when a real CoH parsing session agreed to "basics" mode and then produced overwhelming amounts of pre-conclusion reasoning anyway — the v2.48.0 Step 2c described two modes but did not enforce them. The v2.52.0 rewrite adds: hard word budgets per turn type (1 sentence for procedural confirmations, ≤60 words for status updates, ≤100 for questions, ≤200 for findings, 0 for re-explanation); a "bottom line first" structural rule; a 12-item DO-NOT list naming each anti-pattern explicitly (pre-tool-call narration, between-tool-call narration, recap of work just done, restating context, etc.); a 6-question pre-send SELF-CHECK the agent runs mentally on every basics-mode draft; an "always rendered" carve-out for questions, blocking errors, and commit SHAs; and a "switch-tighter" signal for when the user says output is still too long. Documentation-only — no schema, validator, or emulator change. v2.51.0 closed CoH Gap 4 (Rule 36 extension v2.51.0 closes CoH Gap 4 — round_script cross-round persistence — and retracts the v2.50.0 documentation claim that "round_script is faithful here," which was empirically false at CLI v3.30.0 / HTML v3.26.0. Three new fields on the per-round Lua `combat` table: `combat.vars` (read/write per-fight scratch table, persisted back to state.combat.vars after each round and round-tripped through state.activeCombat across Rule 46 pause/resume); `combat.wounds_dealt` and `combat.wounds_taken` (read-only engine-maintained monotonic counters incremented from the prior round's `last_result` tag). The canonical CoH §263 Manic Beast rage-buff shape ships as a round_script + `combat.vars` recipe in the codex doc — declarative `on_round_start` triggered_effects shape is filed for future consideration. No schema change (runtime Lua context only); pre-v2.51 round_scripts unaffected. v2.50.0 closed CoH Gaps 1 & 2 — `applies_on: "double"` keyword + `instant_death` effect verb. v2.49.0 added Step 2d "Spoiler Level" — no-spoilers / reduce-spoilers / spoilers-OK, a session-opening preference orthogonal to verbosity that controls how much story content the agent surfaces to the user. No-spoilers restricts agent output to rules + front matter + section numbers + shape-only mechanical descriptions; reduce-spoilers uses general categorical language and offers a substitution-or-deferral menu for inherently spoilery questions; spoilers-OK is the pre-Step-2d behavior. The agent logs no-spoilers best-judgment calls to `known_issues.md` so the user can review them post-play. Companion §12.0 sub-section in the remediation chapter re-confirms BOTH preferences at pass start (remediation may happen in a different session than the initial parse, so a refresh is mandatory) and gives a per-finding-category framings table showing how the same finding looks at each spoiler level. Documentation-only — no schema, validator, or emulator change. v2.48.0 added Step 2c "Output Verbosity Mode" — verbose (default) vs just-the-basics, a session-opening preference that controls how the agent presents progress to the user. Verbose narrates reasoning, surfaces tool calls and file references, gives running updates; basics suppresses the monologue and shows only questions, bottom-line conclusions, and just enough context to inform a decision. Mode is sticky for the session, switchable any time by typing "verbose" or "basics". Internal tool calls and parsing quality are identical between modes. Companion sub-section "Visual question affordance" mandates that every user-facing question be either an `AskUserQuestion` tool call (preferred for choices with discrete options — renders as selectable chips) or **bold markdown** (for free-form answers), so users never have to scan paragraphs of monologue to find the question. Both guidelines apply during interactive parsing sessions; documentation-only — no schema or emulator change. v2.47.0 added Rule 49.1's stat-name-in-note catch-net. v2.45.0 / v1.33.0 / v3.28.0 added Rule 49 multi-stat `failure_penalty` array + `checkMechanicVerbsInNote` catch-net + extended disarmament regex set (FF-dialect Equipment List / leave behind / in exchange) — plus a HTML-emulator bugfix removing a broken `penalty.type === 'modify_stat'` gate that had silently dropped all single-stat penalties in the browser; v2.44.0 / v1.32.0 / v3.27.0 added Rule 48 `interrupt_after_rounds` round-cap interrupt; v2.43.0 / v1.31.0 / v3.25.0 added Rule 47 `route_by_flag`, the §12.14 play-execution gate, the §12.15 resume-after-engine-update flow, and the top-level `schema_version` field; v2.42.0 / v1.30.0 / v3.24.0 added Rule 46 pausable/resumable combat as active state. HTML emulator v3.23.0 wired up Rule 40 `choose_items mode:"remove"`; still pending Rule 42 wire-up AND the chargen `roll_table` action fix from prior bumps; see CHANGELOG).
